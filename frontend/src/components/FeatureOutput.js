...
import { Download, Copy, Check, ArrowLeft, FileCode2, Zap, Play } from 'lucide-react';
import './FeatureOutput.css';

export default function FeatureOutput({ output, onDownload, onReset, onSendToRunner }) {
  ...
  return (
    <div className="space-y-4 pt-4">
      ...
      <div className="flex flex-wrap gap-2">
        ...
        <button className="btn btn--ghost" onClick={onSendToRunner}>
          <Play size={14} /> Enviar al Runner
        </button>
        <button className={`btn ${copied ? 'btn--success' : 'btn--ghost'}`} onClick={handleCopy}>
          {copied ? <><Check size={14} /> Copiado</> : <><Copy size={14} /> Copiar codigo</>}
        </button>
      ...
      <div className="card">
        <p className="card__title"><Zap size={14} /> Como ejecutar este test</p>
        <ol className="space-y-3 list-decimal list-inside text-sm text-slate-400">
          <li>
            Descarga el archivo y copialo a <code className="px-1.5 py-0.5 rounded bg-[#0b0f1a] text-violet-300 text-xs font-mono border border-[#1e293b]">src/test/resources/</code>
          </li>
          <li>
            Defini <code className="px-1.5 py-0.5 rounded bg-[#0b0f1a] text-violet-300 text-xs font-mono border border-[#1e293b]">baseUrl</code> en tu{' '}
          ...
          <li>
            Ejecuta con Maven: <code className="px-1.5 py-0.5 rounded bg-[#0b0f1a] text-emerald-300 text-xs font-mono border border-[#1e293b]">mvn test -Dtest=KarateRunner -Dkarate.env=qa</code>
          </li>
        </ol>
      </div>
    </div>
